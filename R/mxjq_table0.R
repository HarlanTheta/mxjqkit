#' @export
mxjq_table0 <- function(data, varat, rn = NULL){
  tab1 <- list()
  for (i in varat) {
    datai <- data[i]
    colnames(datai) <- "X"
    if (is.factor(datai[[1]])) {
      ...
      if(is.null(rn)){
        rn <- mxjq_signif(datai2)
      }
      if (st$p > 0.1) {
        M = round(mean(datai[[1]], na.rm = T), rn)
        S = round(sd(datai[[1]], na.rm = T), rn)
        res1 <- data.frame(
          Xname = colnames(data)[i],
          X = "",
          Stat = paste0(M, "(", S, ")"),
        ...
        M = round(median(datai[[1]], na.rm = T), rn)
        q34 = round(quantile(datai[[1]], 0.75, na.rm = T), rn)
        q14 = round(quantile(datai[[1]], 0.25, na.rm = T), rn)
        S = paste(q14, q34, sep = "-")
        res1 <- data.frame(
          Xname = colnames(data)[i],
          X = "",
