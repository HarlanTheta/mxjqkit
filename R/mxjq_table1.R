#' @export
mxjq_table1 <- function(data, varat, groupat, rn = NULL){
  tab1 <- list()
  for (i in varat) {
    ...
    if (is.factor(datai[[1]])) {
      ...
      if(is.null(rn)){
        rn <- mxjq_signif(datai3$X)
      }
      if (all(st$p > 0.1)) {
        res1 <- datai3 %>%
          group_by(Group) %>%
          summarise(M = round(mean(X, na.rm = T), rn),
                    S = round(sd(X, na.rm = T), rn)) %>%
          ungroup() %>%
        ...
        res1 <- datai3 %>%
          group_by(Group) %>%
          summarise(M = round(median(X, na.rm = T), rn),
                    S = paste(
                      round(quantile(X, 0.25, na.rm = T), rn),
                      round(quantile(X, 0.75, na.rm = T), rn),
                      sep = "-"
                    )) %>%
          ungroup() %>%
          mutate(ms = paste0(M, "(", S, ")"), 
